...
private:
	...
	void PrintElement() const noexcept(false) {
		...
		printf("[%.*s][%d][%zu][%s][%s][%d]\n",
			...
			typeid(a).name(),
			MTString(a).c_str(),
			0//LogMessageFormatTemplate::ArgFormatMatch_v<f, decltype(a)>
		);
		PrintElement<s + 1>();
	}
...
void f(T, Args...args) {
	printf("%s:%d/%d\n",
		...
		ValidateLogArguments<T,int,double>::value
	);
	f(args...);
}
...
const auto CreateLogMessage(const LogMessageFormatTemplate& _lt, Args&&... args) {
...
int main()
{
	...
	try
	{
		...
		int i1 = 1;
		const auto lm1 = CreateLogMessage<lt1.PlaceholderCount()>(lt1, i1, 2ULL, 3.3, "literal", temp);
		lm1.DoDumbPrint();}

		{static constexpr LogMessageFormatTemplate lt2("Goodbye, nothing to say");
		static_assert(lt2.IsValid(), "Invalid string (2)");
		constexpr auto lm2 = CreateLogMessage<lt2.PlaceholderCount()>(lt2);
		lm2.DoDumbPrint();}
